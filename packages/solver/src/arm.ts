...
export interface ArmParams {
  baseHeight: number; // table → shoulder pivot, along +y (mm)
  shoulderOffset: number; // shoulder pivot radial offset from the yaw axis (mm)
  upperArm: number; // L1: shoulder → elbow (mm)
  forearm: number; // L2: elbow → wrist (mm)
  tool: number; // L3: wrist → gripper tip (mm)
  elbowUp?: boolean; // pick the elbow-up IK branch (default true — natural pose)
}

export interface ArmPose {
  reachable: boolean; // target within the annulus [|L1−L2|, L1+L2] of the wrist locus
  yaw: number; // base joint about +y (rad), aims the arm plane at the target
  // absolute link angles in the (radial, height) plane, measured from +radial:
  shoulder: number;
  elbow: number;
  wrist: number;
  // motor (relative) angles — what a joint encoder reads: Δ from the parent link
  joints: { base: number; shoulder: number; elbow: number; wrist: number };
  points: Vec3[]; // world polyline: [base, shoulder, elbow, wrist, tip]
  tip: Vec3; // gripper tip (== target when reachable, top-down approach)
}
...
export function armForward(p: ArmParams, yaw: number, q1: number, q2: number, q3: number): Vec3[] {
  const dirX = Math.sin(yaw), dirZ = Math.cos(yaw); // yaw 0 faces +z
  const toWorld = (r: number, h: number): Vec3 => [r * dirX, h, r * dirZ];
...
export function armInverse(p: ArmParams, target: Vec3): ArmPose {
