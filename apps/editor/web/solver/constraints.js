...
// matcher cannot do.
import { vdot, vnorm, vscale, vsub } from './vec.js';
import { worldDir, worldPoint } from './body.js';
// Two points (one local to each body) must coincide. Removes 3 DOF.
export function coincident(a, localA, b, localB) {
    return {
        kind: 'coincident',
        bodies: [a, b],
...
export function parallel(a, axisA, b, axisB, sense = 1) {
    return {
        kind: 'parallel',
        bodies: [a, b],
        residual: (bs) => {
...
export function concentric(a, lineA, b, lineB, sense = 1) {
    return {
        kind: 'concentric',
        bodies: [a, b],
        residual: (bs) => {
            ...
            const par = [ax[0] - sense * bx[0], ax[1] - sense * bx[1], ax[2] - sense * bx[2]];
...
export function distance(a, localA, b, localB, dist) {
    return {
        kind: 'distance',
        bodies: [a, b],
        residual: (bs) => {
